...
export function InstallPrompt({ locale = 'en' }: InstallPromptProps) {
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      // 保存された表示モードを確認
      const savedMode = localStorage.getItem('xlsx-extract-images-install-prompt-mode') as DisplayMode | null;
      const dismissedTime = localStorage.getItem('xlsx-extract-images-install-prompt-dismissed');

      // 1週間経過していたらリセット
      if (dismissedTime) {
        const oneWeek = 7 * 24 * 60 * 60 * 1000;
        if (Date.now() - parseInt(dismissedTime, 10) >= oneWeek) {
          localStorage.removeItem('xlsx-extract-images-install-prompt-mode');
          localStorage.removeItem('xlsx-extract-images-install-prompt-dismissed');
  ...
  const handleInstall = async () => {
    ...
    setDisplayMode('hidden');
    localStorage.setItem('xlsx-extract-images-install-prompt-mode', 'hidden');
  };

  const handleDismissBanner = () => {
    // バナーを閉じてフッターモードに移行
    setDisplayMode('footer');
    localStorage.setItem('xlsx-extract-images-install-prompt-mode', 'footer');
  };
  ...
  const handleDismissFooter = () => {
    ...
    setDisplayMode('hidden');
    localStorage.setItem('xlsx-extract-images-install-prompt-mode', 'hidden');
    localStorage.setItem('xlsx-extract-images-install-prompt-dismissed', Date.now().toString());
  };
