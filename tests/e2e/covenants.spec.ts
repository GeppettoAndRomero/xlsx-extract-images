...
test.describe('covenants', () => {
  ...
  test('links to related tools and its own engineering-notes article (#177)', async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      expect(href, `related card ${i} is not a self-link`).not.toContain('/xlsx-extract-images/');
    ...
    if (await blogLink.count()) {
      await expect(blogLink).toHaveAttribute('href', /^https:\/\/runlocally\.app\/blog\/xlsx-extract-images\/$/);
    }
  });
  ...
  test('keeps no input data in the page URL after extraction (#7)', async ({ page }) => {
    await page.goto('/xlsx-extract-images/');
  ...
  test('extracts offline after the first online visit (#2)', async ({ page }) => {
    ...
    await page.reload();
    await waitReady(page);
    // Extract online so the zip.js chunk is cached too.
    await extract(page);
    ...
    try {
      ...
      await waitReady(page);
      await extract(page); // extract with no network at all
    } finally {
